...
use App\Filament\Resources\PreventiveResource\RelationManagers;
...
class PreventiveResource extends Resource
{
    ...
    {
        return 'preventivi'; // oppure 'preventivi' se vuoi il plurale corretto in italiano
    }
    //test-1
//test-2
                                                ...
                                                modifyQueryUsing: function ($query) {
                                                    ...
                                                    return $query
                                                        ...
                                                        ->where(function ($q) use ($userId) {
                                                            ...
                                                            ->orWhereHas('agenti_gestori', fn ($agentiQuery) =>
                                                                $agentiQuery->where('user_id', $userId)
                                                            );
                                                        })

                                                        // 🔥 filtro availability attiva
                                                        ->whereHas('agentAvailabilities', function ($q) use ($userId) {
                                                ...
                                                ->getSearchResultsUsing(function (string $search) {
                                                    ...
                                                    return QuoteRequest::query()
                                                        ...
                                                        ->mapWithKeys(fn ($record) => [
                                                            $record->id => "{$record->id} - {$record->oggetto}",
                                                        ]);
...
->extraAttributes([
    'x-data' => '{}',
    'x-init' => '
        $el.querySelector("[wire\\\\:click*=add]")?.addEventListener("click", () => {

            $nextTick(() => {
                window.dispatchEvent(
                    new CustomEvent("repeater-collapse", {
                        detail: "data.extra_services"
                    })
                )
            })
                                    ...
                                    ->schema([
                                        ...
                                        ->options(function (Get $get) {
                                            ...
                                            return collect($tipologie)
                                                ->mapWithKeys(function ($tipo) use ($allOptions) {
                                                    return [
                                                        $tipo => $allOptions[$tipo] ?? ucwords(str_replace('_', ' ', $tipo))
                                                    ];
                                                })
                                                ->toArray();
                                        })
                                        ...
                                        ->afterStateUpdated(function ($state, callable $set, Get $get) {
                                            ...
                                            if (!empty($state)) {
                                                ...
                                                $tipi = is_array($state) ? $state : [$state];

                                                $query = \App\Models\ExtraService::where(function ($query) use ($tipi) {

                                                    foreach ($tipi as $t) {
                                                        $query->orWhere('tipo', 'LIKE', '%"'.$t.'"%')
                                                            ->orWhere('tipo', $t);
                                                    }
                                        ...
                                        ->afterStateHydrated(function ($set, $state, $record) {
                                            ...
                                            if ($record && $record->extra_service) {
                                                ...
                                                $tipo = $record->extra_service->tipo;

                                                $decoded = json_decode($tipo, true);

                                                $set('tipo', $decoded ?? $tipo);
                                        ...
                                        ->createOptionUsing(function (array $data, Get $get): string {
                                            ...
                                            \App\Models\ExtraService::create([
                                                ...
                                                'tipo' => $data['tipo'],
                                                'nome' => $data['nome'] ?? '',
                                                'descrizione_servizio' => $data['descrizione_servizio'] ?? null,
    ...
    ->options(function (Get $get) {
        ...
        if (blank($tipo)) {
            return Supplier::query()
                ...
                ->orderBy('cognome')
                ->get()
                ->mapWithKeys(fn($supplier) => [$supplier->id => trim("{$supplier->nome} {$supplier->cognome}")])
                ->toArray();
        }

        // Normalizza tipo come array (anche se è una stringa singola)
        $tipi = is_array($tipo) ? $tipo : [$tipo];

        // Recupera gli ID dei fornitori che hanno almeno una tipologia corrispondente
        $supplierIds = \App\Models\ExtraService::whereNotNull('supplier_id')
            ->where(function ($query) use ($tipi) {
                foreach ($tipi as $t) {
                    $query->orWhere(function ($q) use ($t) {
                        $q->where('tipo', 'LIKE', '%"'.$t.'"%') // JSON array
                          ->orWhere('tipo', $t);                // stringa singola
                    });
                }
            })
            ->distinct()
    ...
    ->afterStateUpdated(function ($state, callable $set, Get $get) {
        ...
        if (!blank($state) && !blank($tipo)) {
            $tipi = is_array($tipo) ? $tipo : [$tipo];

            $service = \App\Models\ExtraService::where('supplier_id', $state)
                ->where(function ($query) use ($tipi) {
                    foreach ($tipi as $t) {
                        $query->orWhere(function ($q) use ($t) {
                            $q->where('tipo', 'LIKE', '%"'.$t.'"%')
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->recordUrl(
                ...
                ? PreventiveResource::getUrl('edit', ['record' => $record->id]) // admin e superadmin -> edit
                : (
                    auth()->user()->hasRole('agente') && $record->created_by === auth()->id()
                    ? PreventiveResource::getUrl('edit', ['record' => $record->id]) // agente -> edit solo se proprio
                    : PreventiveResource::getUrl('view', ['record' => $record->id]) // altri -> rotta pubblica
                )
            )
            ->columns([
                ...
                Tables\Columns\TextColumn::make('tag')
                    ->label('Tag')
                    ->limit(20)
                    ->tooltip(fn($record) => $record->tag) // mostra tutta la descrizione al passaggio del mouse
                    ->searchable(),
    ...
    public static function getPages(): array
    {
        return [
            ...
            'view' => Pages\ViewPreventive::route('/{record}/view'),
        ];
    }
